...
function the_posts_navigation() {
	...
	<nav class="navigation posts-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php _e( 'Posts navigation', 'zacklive' ); ?></h2>
		<div class="nav-links">

			<?php if ( get_next_posts_link() ) : ?>
			<div class="nav-previous"><?php next_posts_link( __( 'Older posts', 'zacklive' ) ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next"><?php previous_posts_link( __( 'Newer posts', 'zacklive' ) ); ?></div>
			<?php endif; ?>
...
function zacklive_posted_on() {
	...
	$posted_on = sprintf(
		_x( 'Posted on %s', 'post date', 'zacklive' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		_x( 'by %s', 'post author', 'zacklive' ),
...
function zacklive_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		if ( $categories_list && zacklive_categorized_blog() ) {
			printf( '<span class="cat-links">' . __( 'Posted in %1$s', 'zacklive' ) . '</span>', $categories_list );
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', __( ', ', 'zacklive' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . __( 'Tagged %1$s', 'zacklive' ) . '</span>', $tags_list );
		}
	}
	...
	edit_post_link( __( '<span class="fa fa-edit"></span> Edit', 'zacklive' ), '<span class="edit-link">', '</span>' );
}
